...
import { envoyerMailAvecRemiseParLigneEnPourcentage, listFactureDetails, updateFacture } from '../../redux/actions/servantActions/factureAction';
  ...
  dispatch(
    updateFacture({
     ...
     const envoyerMailAvecRemiseParLigne = async()=>{
      ...
      try {
        await envoyerMailAvecRemiseParLigneEnPourcentage(id, token)(dispatch);
      } catch (error) {
...
<td> 
                <Typography style={{marginLeft:"75px", width:"60px",fontSize: "14px"}}>{facture.remiseParLignePourcent}</Typography>
          ...
          fontSize: "18px",
           ...
           marginLeft:"60px"
            ...
            <div className={styles.summaryItem}>
             ...
             {facture.Acompte2 && facture.Acompte2.slice ? (subTotal - (subTotal * (facture.Acompte2.slice(0, -1) / 100))).toFixed(2) : ""}
             </h4>
             <p> Payé le {echa}</p>
